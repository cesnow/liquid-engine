...
func FindLiquidGuestUser(autoId string) *LiquidUser {
	...
	var user *LiquidUser
	findUserErr := LiquidSDK.GetServer().GetLiquidUserCol().FindOne(nil, filter).Decode(&user)
	if findUserErr != nil {
		return nil
	}
	return user
...
func BindLiquidUser(AutoId string, FromId string, FromType string, FromToken string) (*mongo.UpdateResult, error) {
	...
	setBindResult, setBindResultErr := LiquidSDK.GetServer().GetLiquidUserCol().UpdateOne(
		nil,
		filter,
		setBindData,
	)
